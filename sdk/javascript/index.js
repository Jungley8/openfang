/**
 * @octarq/sdk — Official JavaScript client for the Octarq Agent OS REST API.
 *
 * Usage:
 *   const { Octarq } = require("@octarq/sdk");
 *   const client = new Octarq("http://localhost:3000");
 *
...
class OctarqError extends Error {
  constructor(message, status, body) {
    super(message);
    this.name = "OctarqError";
    this.status = status;
    this.body = body;
  }
}
...
class Octarq {
  /**
   * @param {string} baseUrl - Octarq server URL (e.g. "http://localhost:3000")
   * @param {object} [opts]
  ...
  async _request(method, path, body) {
    ...
    if (!res.ok) {
      ...
      throw new OctarqError("HTTP " + res.status + ": " + text, res.status, text);
    }
...
class AgentResource {
  ...
  async upload(id, file, filename) {
    ...
    if (!res.ok) throw new OctarqError("Upload failed: " + res.status, res.status);
    return res.json();
  }
...
module.exports = { Octarq: Octarq, OctarqError: OctarqError };
