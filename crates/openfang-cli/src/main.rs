...
fn main() {
    ...
    match cli.command {
        ...
        Some(Commands::Telos(sub)) => match sub {
            TelosCommands::Init { quick } => cmd_telos_init(quick),
            TelosCommands::Status => cmd_telos_status(),
            TelosCommands::Edit { file } => cmd_telos_edit(&file),
            TelosCommands::Reload => cmd_telos_reload(),
            TelosCommands::Preview { hand } => cmd_telos_preview(&hand),
            TelosCommands::Export { output } => cmd_telos_export(output.as_deref()),
            TelosCommands::Report { days } => cmd_telos_report(days),
        },
    }
}
...
fn cmd_telos_preview(hand: &str) {
    ...
    // Find the hand in bundled registry
...
fn cmd_telos_export(output: Option<&std::path::Path>) {
    ...
    {
        Ok(pretty) => pretty,
        Err(_) => json_str, // already valid JSON from daemon or local
    };
