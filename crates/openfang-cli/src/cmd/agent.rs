//! Agent commands: new, spawn, list, chat, kill, set. Quick chat and message.

use crate::daemon::{boot_kernel, daemon_client, daemon_json, find_daemon};
use crate::templates;
use crate::ui;
...
pub fn cmd_agent_chat(config: Option<PathBuf>, agent_id_str: &str) {
    crate::tui::chat_runner::run_chat_tui(config, Some(agent_id_str.to_string()));
}
...
pub fn cmd_agent_set(config: Option<PathBuf>, agent_id_str: &str, field: &str, value: &str) {
    match field {
...
pub fn cmd_agent_set_model(config: Option<PathBuf>, agent_id_str: &str, model: &str) {
...
pub fn cmd_agent_new(config: Option<PathBuf>, template_name: Option<String>) {
    ...
    let chosen = match template_name {
        ...
        None => {
            ...
            ui::blank();
            let choice = crate::prompt_input("  Choose template [1]: ");
...
pub fn cmd_quick_chat(config: Option<PathBuf>, agent: Option<String>) {
    crate::tui::chat_runner::run_chat_tui(config, agent);
}

pub fn cmd_message(agent: &str, text: &str, json: bool) {
    let base = crate::daemon::require_daemon("message");
    let client = daemon_client();
    let body = daemon_json(
        client
    ...
    if let Some(reply) = body["reply"].as_str() {
        println!("{reply}");
