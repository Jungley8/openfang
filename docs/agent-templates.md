...
Octarq ships with **30 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
octarq spawn orchestrator
octarq spawn coder
octarq spawn --template agents/writer/agent.toml
```
...
> General-purpose assistant. The default Octarq agent for everyday tasks, questions, and conversations.
...
```bash
octarq spawn meeting-assistant
...
octarq spawn coder --name "backend-coder"

# Spawn from a TOML file path
octarq spawn --template agents/custom/my-agent.toml

# List running agents
octarq agents

# Send a message
octarq message <agent-id> "Write a function to parse TOML files"

# Kill an agent
octarq kill <agent-id>
```
