...
On desktop platforms, `tauri-plugin-single-instance` prevents multiple copies of Octarq from running simultaneously. When a second instance attempts to launch, the existing instance's main window is shown, unminimized, and focused:
...
To actually quit, use the **"Quit Octarq"** option in the system tray menu.
...
Check or toggle whether Octarq launches at OS login. Uses `tauri-plugin-autostart` (launchd on macOS, registry on Windows, systemd on Linux).
