...
Octarq is organized as a Cargo workspace with 14 crates (13 code crates + xtask). Dependencies flow downward (lower crates depend on nothing above them).
...
| Crate                 | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **openfang-types**    | Core type definitions used across all crates. Defines `AgentManifest`, `AgentId`, `Capability`, `Event`, `ToolDefinition`, `KernelConfig`, `OctarqError`, taint tracking (`TaintLabel`, `TaintSet`), Ed25519 manifest signing, model catalog types (`ModelCatalogEntry`, `ProviderInfo`, `ModelTier`), tool compatibility mappings (21 OpenClaw-to-Octarq), MCP/A2A config types, and web config types. All config structs use `#[serde(default)]` for forward-compatible TOML parsing.                                                                                                                                                                                                                                                                                                                 |
...
| **openfang-kernel**   | The central coordinator. `OctarqKernel` assembles all subsystems: `AgentRegistry`, `AgentScheduler`, `CapabilityManager`, `EventBus`, `Supervisor`, `WorkflowEngine`, `TriggerEngine`, `BackgroundExecutor`, `WasmSandbox`, `ModelCatalog`, `MeteringEngine`, `ModelRouter`, `AuthManager` (RBAC), `HeartbeatMonitor`, `SetupWizard`, `SkillRegistry`, MCP connections, and `WebToolsContext`. Implements `KernelHandle` for inter-agent operations. Handles agent spawn/kill, message dispatch, workflow execution, trigger evaluation, capability inheritance validation, and graceful shutdown with state persistence.                                                                                                                                                                                 |
...
When `OctarqKernel::boot_with_config()` is called (either by the daemon or in-process by the CLI/desktop app), the following sequence executes:
...
pub trait LlmDriver: Send + Sync {
    ...
    ) -> Result<LlmResponse, OctarqError>;
...
Octarq implements 16 security systems organized into critical fixes and state-of-the-art defenses:
...
- **FangHub**: Native Octarq marketplace (`FangHubClient`).
...
- **Tool Compat**: 21 OpenClaw-to-Octarq tool name mappings in `tool_compat.rs`.
...
Octarq implements both MCP client and server:
...
- **MCP Server** (`mcp_server.rs`): Exposes Octarq's 23 built-in tools via the MCP protocol. Enables external tools to use Octarq as a tool provider.
...
The Octarq Protocol (OFP) enables peer-to-peer agent communication across machines.
...
```mermaid
     graph TD
         ...
         %% Interface & Protocol
         API --> KERNEL
        WIRE[openfang-wire] -- OFP Protocol --> KERNEL
   
        %% Central Coordinator
        subgraph Core[Kernel & Coordination]
            KERNEL
        end
        ...
        KERNEL --> MIGRATE[openfang-migrate]
   
        %% Execution Engine Details
        RUNTIME --> LLM[LLM Drivers: Anthropic/Gemini/OpenAI]
        RUNTIME --> WASM[WASM Sandbox]
        RUNTIME --> TOOLS[23 Built-in Tools]
        RUNTIME --> MCP[MCP Client/Server]
        ...
        style MEMORY fill:#dfd,stroke:#333
        style TYPES fill:#eee,stroke:#333
```