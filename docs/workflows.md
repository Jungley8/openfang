...
All workflow and trigger CLI commands require a running Octarq daemon.
...
```
octarq trigger list [--agent-id <uuid>]
```
...
octarq trigger create <agent-id> '"lifecycle"' --prompt "Lifecycle: {{event}}"

# Watch for a specific agent spawn
octarq trigger create <agent-id> '{"agent_spawned":{"name_pattern":"coder"}}' --max-fires 1

# Watch for content containing "error"
octarq trigger create <agent-id> '{"content_match":{"substring":"error"}}'
```
